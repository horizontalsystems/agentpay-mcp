...
export const WC_SESSION_DEAD_MESSAGE =
  'WalletConnect session dead on relay (no response from phone). Call get_x402_pairing_link, reconnect Unstoppable Wallet, then retry fetch_paid_service.';

export const NO_ACTIVE_SESSION_MESSAGE =
  'WalletConnect session not active on backend. Call get_x402_pairing_link, send the raw wc: URI, user taps Connect, then retry fetch_paid_service.';
